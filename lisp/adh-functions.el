...
(defun adh-set-font (family size &optional weight)
  (when (display-graphic-p)
    (if (find-font (font-spec :family family))
        (let* ((w (or weight "regular"))
               (spec (format "%s-%s:%s" family size w)))
          (unless (equal (frame-parameter nil 'font) spec)
            (set-frame-font spec nil t)
            (setf (alist-get 'font default-frame-alist nil nil #'equal) spec)))
      (message "[adh] Warning: font family not found: %s" family))))
...
(defun adh-copy-full-path ()
  (interactive)
  (if-let* ((f (adh--file-at-point)))
      (progn (kill-new (expand-file-name f))
             (message "Copied full path: %s" f))
    (user-error "No file to copy here")))
